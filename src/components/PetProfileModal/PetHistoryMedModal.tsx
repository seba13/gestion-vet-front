...
const PetHistoryMedModal: React.FC<PetHistoryMedModalProps> = ({
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        console.log("entra en fetch data");

        const clinicalRecordData: ClinicalRecord[] = await fetchClinicalRecord(
          idMascota
        );
  ...
  useEffect(() => {
    ...
    console.log(clinicalRecord.length);
  }, [clinicalRecord]);
